...
table(members)
head(members)
customers_sample$group = members
head(customers_sample)
# Show profile of each segment
aggregate(customers_sample[, 2:4], by = list(members), mean)
#lisäsin omaa toisen kurssin esimerkkeihin pohjautuen, esim. prosenttiosuus ja muutin koodin käyttämään ~-merkkiä
alfa = aggregate(cbind(recency, frequency, amount) ~ group, data = customers_sample, FUN = mean)
proptemp = aggregate(customer_id ~ group, data = customers_sample , FUN = length)
prop = proptemp$customer_id / sum(length(customers_sample$customer_id))
alfa$proportion = prop
alfa = alfa[order(alfa$proportion, decreasing = TRUE),]
